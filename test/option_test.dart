...
void main() {
  group('Option', () {
    ...
    test('filtering options', () {
      ...
      expect(Option.none().filter(((v) => fail('oh no')) as bool Function(dynamic)), isA<None>());
    });
    ...
    test('this and that', () {
      ...
      expect(Option.some(2).andThen((v) => Option.some(v * 2)).unwrap(),
          equals(4));
      expect(Option.none().andThen(((v) => fail('oh no')) as Option<dynamic> Function(dynamic)), isA<None>());
    });

    test('this or that', () {
      expect(Option.some(2).or(Option.none()), isA<Some>());
      expect(Option.none().or(Option.some(2)), isA<Some>());
      expect(Option.some(2).orElse((() => fail('oh no')) as Option<int> Function()), isA<Some>());
